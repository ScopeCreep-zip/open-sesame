...
**attest**: Downloads all `.deb` and `.rpm` artifacts and generates SLSA build provenance
attestations using `actions/attest-build-provenance@v4`.

**upload-assets**: Downloads `.deb` and `.rpm` artifacts, generates `SHA256SUMS.txt` covering
all packages, renders install instructions from a template with per-architecture checksums for
both formats, and uploads all `.deb`, `.rpm` files and checksums to the GitHub release.

**publish**: Downloads `.deb` and `.rpm` artifacts and documentation, imports the GPG signing key,
generates the APT repository via `mise run ci:release:apt-repo` and the DNF repository via
`mise run ci:release:rpm-repo`, merges both repository trees with documentation into a single
...
`actions/attest-build-provenance@v4`. This runs in the `attest` job after both deb and rpm builds
